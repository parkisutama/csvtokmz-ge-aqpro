...
def create_kml(df: pd.DataFrame, output_kml: str, folder_name: str):
    ...
    for _, row in df.iterrows():
        point = folder.newpoint(name=row.get('name', ''), description=row.get('description', ''))
        point.coords = [(row['longitude'], row['latitude'])]

        # Attach photo
        if pd.notnull(row.get('photo_path')):
            photo_filename = os.path.basename(row['photo_path'])
            point.description += f"<br /><img width='480' src='files/{photo_filename}'><br />"
        ...
        for column in ['photo_path', 'icon_url']:
            for file_path in df[column].dropna().unique():
                if os.path.exists(file_path):
                    zipf.write(file_path, arcname=f"files/{os.path.basename(file_path)}")

    os.remove(kml_file)


def generate_kmz_from_csv(csv_file: str):
    """
    Main function to generate a KMZ file from a CSV.
    """
    parent_folder = os.path.join(OUTPUT_FOLDER, os.path.splitext(os.path.basename(csv_file))[0])
